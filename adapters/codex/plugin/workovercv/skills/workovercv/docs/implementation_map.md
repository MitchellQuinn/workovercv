...
| Report rendering | `src/workovercv/render.py`, `src/workovercv/pdf.py` | Renders audit-oriented `report.md`, human-facing `summary-report.md`, and `screening_brief.md` from v0.6 `report.json`, then writes matching PDF artifacts from the generated Markdown. |
| Final validation | `src/workovercv/validation.py` | Checks required artifacts, PDF header validity, record references, v0.6 report shape, gap follow-ups, role-family references and caveats, raw chronology leakage, red-team status, prohibited wording, hiring-decision language, and legacy heading regressions. |
