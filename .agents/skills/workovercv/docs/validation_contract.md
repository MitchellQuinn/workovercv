...
the final JSON and JSONL artifacts, and PDF validation covers generated PDF
presence and file headers.
...
- PDF validation implementation: `src/workovercv/validation.py::_validate_pdf_artifacts`
  rejects empty files and files that do not start with `%PDF-`.
...
| PDF artifact validation | `src/workovercv/render.py` writes PDF artifacts through `src/workovercv/pdf.py`; `src/workovercv/validation.py::_validate_pdf_artifacts` checks required PDF files are non-empty and start with `%PDF-`. | Not schema-controlled; PDFs are binary derivatives of validated Markdown. | `tests/test_render_validate.py::test_render_report_outputs_full_and_summary_reports_without_legacy_sections`, `test_validate_run_rejects_missing_pdf_artifact`, `test_validate_run_rejects_invalid_pdf_artifact`; `tests/test_scan.py::test_scan_local_path_writes_valid_run`. |
